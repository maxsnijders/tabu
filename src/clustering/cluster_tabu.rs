...
pub fn cluster_tabu<Item>(
...
) -> Vec<Vec<Item>> where Item : Clone + Eq + Hash + PartialEq {
    if n_clusters == 1 {
        return vec![items];
    }
    ...
    fn descendants<Item>(state: &State<Item>) -> impl Iterator<Item = State<Item>> where Item : Clone + PartialEq {
    ...
    let initial_state = State {
        clusters: vec![items].into_iter().chain(vec![vec![]; n_clusters - 1]).collect()
    };
...
mod tests {
    ...
    fn test_clustering_with_diameter_cost(){
        ...
        let mut clusters: Vec<Vec<i32>> = cluster_tabu(items, |clusters: &Vec<Vec<i32>>| {
            clusters.iter().map(|cluster| diameter(cluster, |x, y| (x - y).abs() as f64)).fold(f64::NEG_INFINITY, f64::max)
        }, n_clusters, max_iterations, stopping_cost).into_iter().map(|mut cluster| {
            cluster.sort();
            cluster
        }).collect();
        clusters.sort();
